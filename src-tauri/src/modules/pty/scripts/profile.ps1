# teraxlyst-shell-integration (PowerShell)
# Emits OSC 7 (cwd) + OSC 133 A/B/D so the host tracks cwd and prompt boundaries.

if ($global:__TERAXLYST_HOOKS_LOADED) { return }
$global:__TERAXLYST_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__teraxlyst_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__teraxlyst_urlencode {
    param([string]$s)
...
function global:prompt {
    ...
    if ($loc.Provider.Name -eq 'FileSystem') {
        ...
        $cwdEnc = __teraxlyst_urlencode $cwd
        $hostName = [System.Environment]::MachineName
        $osc7 = "$esc]7;file://$hostName$cwdEnc$esc\"
    }

    $original = if (Test-Path Function:__teraxlyst_user_prompt) {
        try { & __teraxlyst_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
