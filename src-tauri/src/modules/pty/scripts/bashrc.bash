# teraxlyst-shell-integration (bashrc)
#
...
if [ -z "$__TERAXLYST_HOOKS_LOADED" ]; then
  ...
  _teraxlyst_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_teraxlyst_urlencode "$PWD")"
    if [ -z "$__TERAXLYST_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __TERAXLYST_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_teraxlyst_precmd:"*) ;;
    *) PROMPT_COMMAND="_teraxlyst_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
