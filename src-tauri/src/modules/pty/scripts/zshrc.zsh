# teraxlyst-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_teraxlyst_ret`.

{
  _teraxlyst_user_zdotdir="${TERAXLYST_USER_ZDOTDIR:-$HOME}"
  [ -f "$_teraxlyst_user_zdotdir/.zshrc" ] && source "$_teraxlyst_user_zdotdir/.zshrc"
...
if [[ -z "$__TERAXLYST_HOOKS_LOADED" ]]; then
  __TERAXLYST_HOOKS_LOADED=1
  ...
  _teraxlyst_precmd() {
    local _teraxlyst_ret=$?
    printf '\e]133;D;%s\e\\' "$_teraxlyst_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_teraxlyst_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _teraxlyst_precmd
    add-zsh-hook preexec _teraxlyst_preexec
  fi
